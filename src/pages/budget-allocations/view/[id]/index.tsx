...
import { getBudgetAllocationById } from 'apiSdk/budget-allocations';
import { BudgetAllocationInterface } from 'interfaces/budget-allocation';

function BudgetAllocationViewPage() {
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Budget Allocation Details',
              isCurrent: true,
            },
          ]}
        />
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('budget_allocation', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/budget-allocations/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('travel_plan', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Travel Plan"
                    text={
                      <Link as={NextLink} href={`/travel-plans/view/${data?.travel_plan?.id}`}>
                        {data?.travel_plan?.destination}
                      </Link>
                    }
                  />
                )}

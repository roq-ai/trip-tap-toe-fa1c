...
import { getBudgetAllocations } from 'apiSdk/budget-allocations';
...
import { getPersonalDetails } from 'apiSdk/personal-details';
...
function DashboardPage() {
  ...
  const budgetAllocationStat = useSWR<PaginatedInterface<BudgetAllocationInterface>>(
    () => `/budget-allocations`,
    () => getBudgetAllocations({ limit: 1, offset: 0 }),
  );

  const travelPlanStat = useSWR<PaginatedInterface<TravelPlanInterface>>(
    () => `/travel-plans`,
    () => getTravelPlans({ limit: 1, offset: 0 }),
  );

  const personalDetailStat = useSWR<PaginatedInterface<PersonalDetailInterface>>(
    () => `/personal-details`,
    () => getPersonalDetails({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Budget Allocations',
      result: budgetAllocationStat?.data?.totalCount,
      isLoading: budgetAllocationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Travel Plans',
      result: travelPlanStat?.data?.totalCount,
      isLoading: travelPlanStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Personal Details',
      result: personalDetailStat?.data?.totalCount,
      isLoading: personalDetailStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
